...
class FinancialController:
    ...
    async def __query_parameters_pharmacy(self, parameters: PharmacyParametersApiSchema) ->  list[Optional[PharmacySchema]]:
        async with session() as s:
            query = await s.execute(select(self.table).where(or_(
    ...
    async def __query_parameters_patient(self, parameters: PatientParametersApiSchema) -> list[Optional[PatientSchema]]:
        async with session() as s:
            query = await s.execute(select(self.table).where(or_(
    ...
    async def query_transaction(self, parameter: Optional[TransactionParametersApiSchema] = None) -> list:
        if parameter:
            return await self.__query_parameters_transaction(parameter)
        return await self.__select_all_transaction()

    async def __query_parameters_transaction(self, parameters: TransactionParametersApiSchema) -> list[Optional[TransactionSchema]]:
        async with session() as s:
            query = await s.execute(select(Transactions, Patients, Pharmacies).join(Transactions.patient).join(Transactions.pharmacy).where(or_(
